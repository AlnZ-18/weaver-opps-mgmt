...
const connectDB = async () => {
  try {
    const conn = await mongoose.connect(process.env.MONGODB_URI, {
      // Modern options (enabled by default in Mongoose 6+, but good practice to show structure)
      autoIndex: true,
    });
...
// Listen to connection state events for debugging and system monitoring
mongoose.connection.on('disconnected', () => {
  console.warn('\x1b[33m[Database] Warning: MongoDB lost connection. Attempting reconnection...\x1b[0m');
});
